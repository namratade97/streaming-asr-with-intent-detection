...
class Zipformer2(EncoderInterface):
    ...
    def streaming_forward(
        ...
        # assuming cls_token is always the first token -

        cls_token = x[0:1, :, :]  # Shape: (1, batch_size, feature_dim) ##### v2
        audio_input = x[1:, :, :]       # Shape: (T, batch_size, feature_dim) ##### v2

        logging.info(f"CLS token at start of Zipformer2.streaming_forward: {cls_token.shape}")
        logging.info(f"CLS token values at start of Zipformer2.streaming_forward: {cls_token.data}")
        ...
        for i, module in enumerate(self.encoders):
            ...
            ds = self.downsampling_factor[i]
            audio_input = convert_num_channels(audio_input, self.encoder_dim[i]) ##### v2

            ###############
            # cls_mask = src_key_padding_mask[:, :1] ##### v2
            audio_mask = src_key_padding_mask[:, 1:]
            audio_mask_downsampled = audio_mask[:, ::ds]
            # new_mask = torch.cat([cls_mask, audio_mask_downsampled], dim=1) ##### v2
            ###############

            audio_input, new_layer_states = module.streaming_forward(
                audio_input,
                states=states[layer_offset * 6 : (layer_offset + num_layers) * 6],
                left_context_len=self.left_context_frames[0] // ds,
                # src_key_padding_mask=src_key_padding_mask[..., ::ds],
                # src_key_padding_mask=new_mask, ##### v2
                src_key_padding_mask=audio_mask_downsampled,  ##### v2
            )

            layer_offset += num_layers
            
            # outputs.append(x) ##### v2
            outputs.append(audio_input) ##### v2
        ...
        # from different pieces of 'outputs', taking each dimension from the
        # most recent output that has it present.
        # x = self._get_full_dim_output(outputs)  ##### v2

        ##################
        # x = self.downsample_output(x)

        # cls_token = x[0:1, :, :]            # (1, batch_size, feature_dim)  ##### v2
        # rest_tokens = x[1:, :, :]  ##### v2
        # downsampled_rest = self.downsample_output(rest_tokens)  ##### v2
        # downsampled_rest = self.downsample_output(rest_tokens)  ##### v2

        downsampled_audio = self.downsample_output(self._get_full_dim_output(outputs))



        x = torch.cat([cls_token, downsampled_audio], dim=0) ##### v2
        ...
        if torch.jit.is_scripting() or torch.jit.is_tracing():
            # lengths = (x_lens + 1) // 2  #####v2
            lengths = (x_lens) // 2 # since audio_len = x_len - 1
        else:
            with warnings.catch_warnings():
                warnings.simplefilter("ignore")
                # lengths = (x_lens + 1) // 2  #####v2
...
class DownsampledZipformer2Encoder(nn.Module):
    ...
    def streaming_forward(
        ...
        cls_token = src[0:1, :, :]
        src_orig_audio = src[1:, :, :]
        
        # # separate the padding mask
        # # src_key_padding_mask_audio = src_key_padding_mask[:, 1:]
        # audio_mask_downsampled = src_key_padding_mask_audio[..., ::self.downsample_factor]
        # src_key_padding_mask_audio = audio_mask_downsampled

        # handle padding mask
        # cls_mask = src_key_padding_mask[:, :1]  # CLS
        audio_mask = src_key_padding_mask[:, 1:]
        audio_mask_downsampled = audio_mask[..., ::self.downsample_factor]
        # src_key_padding_mask_audio = torch.cat([cls_mask, audio_mask_downsampled], dim=1)


        # apply downsampling only to the audio frames
        src_downsampled_audio = self.downsample(src_orig_audio)

        ################

        # src, new_states = self.encoder.streaming_forward(
        src_processed_audio, new_states = self.encoder.streaming_forward(
            # src,
            src_downsampled_audio,
            states=states,
            left_context_len=left_context_len,
            # src_key_padding_mask=src_key_padding_mask,
            # src_key_padding_mask=src_key_padding_mask_audio[:, 1:],  # exclude CLS for encoder ####v2
            src_key_padding_mask=audio_mask_downsampled,  # exclude CLS for encoder ####v2
        )
        
        ############
        # src = self.upsample(src)

        # upsample the processed audio frames
        src_upsampled_audio = self.upsample(src_processed_audio)
        src_upsampled_audio = src_upsampled_audio[:src_orig_audio.shape[0]]

        # re-concatenate the CLS token with the processed audio
        src_combined = torch.cat([cls_token, src_upsampled_audio], dim=0)
        ############


        # remove any extra frames that are not a multiple of downsample_factor
        # src = src[: src_orig.shape[0]]
        # src = src[: src_orig_audio.shape[0]]

        return self.out_combiner(src_orig, src_combined), new_states
