...
class AsrModel(nn.Module):
    def __init__(
        ...
        # self.cls_token = nn.Parameter(torch.randn(1, 1, 1)) #dummy dim
        # self.cls_token_initialized = False
        # self.cls_token = nn.Parameter(torch.randn(1, 1, encoder_dim))

        # embed_dim = self.encoder_embed.out_channels  # fixed int

        def _to_int_tuple(s: str):
            return tuple(map(int, s.split(",")))

        # embed_dim = _to_int_tuple(encoder_dim)[0]
        # embed_dim = encoder_dim
        embed_dim = encoder_embed.out.out_features  
    ...
    def forward_encoder(
        self, x: torch.Tensor, x_lens: torch.Tensor, supervision_texts: List[str], is_evaluation=False
        ...
        encoder_embed_dim = x.size(-1)
        # logging.info(f"Memory allocated after encoder_embed: {torch.cuda.memory_allocated() // 1000000}M")

        # Step 2: Prepend the [CLS] token.
        # Expand the single CLS token to match the batch size (N).
        # x.shape becomes (N, T, C), cls_token.shape becomes (N, 1, C)

        # if not self.cls_token_initialized:
        #     # resize CLS token once and move to correct device
        #     self.cls_token.data = torch.randn(1, 1, encoder_embed_dim, device=x.device)
        #     self.cls_token_initialized = True

        # if not hasattr(self, 'cls_token') or self.cls_token.size(-1) != encoder_embed_dim:
        #     # Create a learnable CLS token with the correct dimension
        #     self.cls_token = nn.Parameter(torch.randn(1, 1, encoder_embed_dim, device=x.device))
        
        cls_token = self.cls_token.expand(x.size(0), -1, -1)


        ################
        logging.info(f"CLS token BEFORE encoder: {self.cls_token.data}")
        logging.info(f"Expanded CLS token shape: {cls_token.shape}")
        ################
        
        
        # concatenate the CLS token at the beginning 
        # x.shape is now (N, T+1, C).
        x = torch.cat([cls_token, x], dim=1)
        
        # update the sequence lengths and create a new padding mask
        x_lens = x_lens + 1
        src_key_padding_mask = make_pad_mask(x_lens)
        
        # pass the modified input through the main encoder...
        x = x.permute(1, 0, 2)  # (N, T+1, C) -> (T+1, N, C)
        encoder_out, encoder_out_lens = self.encoder(x, x_lens, src_key_padding_mask)
        encoder_out = encoder_out.permute(1, 0, 2) # (T+1, N, C) -> (N, T+1, C)
        
        # extract the CLS token's final representation for intent classification
        # CLS token is at index 0 of the sequence dimension.
        cls_output = encoder_out[:, 0, :] # Shape: (N, encoder_dim)


        logging.info(f"CLS token AFTER encoder: {cls_output}")
        ...
        if not is_evaluation:
            # --- Start Intent Loss Calculation ---
            intent_tokens = [extract_intent_from_supervision(text) or " <unk>" for text in supervision_texts]

            valid_intent_tokens = [token for token in intent_tokens if token in some_mapping] ##

            valid_supervision_texts_indices = [i for i, token in enumerate(intent_tokens) if token in some_mapping]
        
            if valid_intent_tokens:
                intent_indices = [some_mapping[token] for token in valid_intent_tokens]
                intent_labels = torch.tensor(intent_indices, device=encoder_out.device)
                valid_logits = intent_logits[valid_supervision_texts_indices]
                
                intent_loss = F.cross_entropy(valid_logits, intent_labels)
            else:
                # if no known intents in batch, set loss to 0
                intent_loss = torch.tensor(0.0, device=encoder_out.device, requires_grad=True)
                

            # intent_indices = [some_mapping[token] for token in intent_tokens]
            
            # intent_labels = torch.tensor(intent_indices, device=encoder_out.device)  # Shape: (N,)

            # intent_loss = None
            # if intent_labels is not None:
            #     # Use the correct logits shape: (N, num_classes)
            #     intent_loss = F.cross_entropy(intent_logits, intent_labels)
                
            # --- End Intent Loss Calculation ---
        
        assert torch.all(encoder_out_lens > 0), (x_lens, encoder_out_lens)
        
        # Step 7: Return the original encoder outputs (excluding the CLS token) for ASR.
        return encoder_out[:, 1:, :], encoder_out_lens - 1, intent_loss, intent_logits
    ...
    def forward(
        ...
        # Compute encoder outputs
        encoder_out, encoder_out_lens, intent_loss, intent_logits = self.forward_encoder(x, x_lens, supervision_texts)
