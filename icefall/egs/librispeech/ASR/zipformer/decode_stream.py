...
class DecodeStream(object):
    def __init__(
        ...
        n_intent = 92
        self.intent_logit_sum = torch.zeros(n_intent, device=device)
        self.total_frames = 0
        ...
        self.intent_mean_logits = None # 1‑D tensor, size = n_intent
        self.intent_probs       = None # softmax over intents
        self.intent_top3        = None # list[(id, prob)]
        self.frame_rels         = [] # per‑chunk relevance list[Tensor(T, n_intents)]

        self.token_times: list[int] = []  # we use this to get the absolute frame instead of guessing it heuristically
        self.token_syms:  list[int] = []

        self.intent_history: List[Tuple[int, List[float]]] = []
