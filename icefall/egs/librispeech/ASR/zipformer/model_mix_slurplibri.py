...
class AsrModel(nn.Module):
    ...
    def forward_encoder(
        ...
        x = x[valid]
        x_lens = x_lens[valid]
        supervision_texts = [text for i, text in enumerate(supervision_texts) if valid[i]]
        ...
        mask = torch.arange(max_len, device= encoder_out.device).unsqueeze(0) < encoder_out_lens.unsqueeze(1) # Now shape: (N, T)
        ...
        #     [some_mapping[tok] for tok in intent_tokens if tok is not None],
        #     device=encoder_out.device
        # )

        intent_labels = torch.tensor(
            [some_mapping[t] if t is not None else -1 for t in intent_tokens], # since we use both Libri and Slurp for finetuning
            device=encoder_out.device,
            dtype=torch.long,
        )  # (N,)
        ...
        return encoder_out, encoder_out_lens, losses
