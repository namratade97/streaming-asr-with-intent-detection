...
class AsrModel(nn.Module):
    ...
    def forward_encoder(
        ...
        logging.info("mean_intent_logits.shape: %s", mean_intent_logits.shape)
        logging.info("intent_labels.shape: %s", intent_labels.shape)
        logging.info("mean_intent_logits: %s", mean_intent_logits.detach().cpu().numpy())
        logging.info("intent_labels: %s", intent_labels.detach().cpu().numpy())
        ...
        if self.focal_loss is not None:
            intent_loss = self.focal_loss(mean_intent_logits, intent_labels)
        ...
        # Check the shapes of the inputs just before calling FocalLoss
        logging.info(f"Shape of mean_intent_logits: {mean_intent_logits.shape}")
        logging.info(f"Shape of intent_labels: {intent_labels.shape}")
        logging.info(f"Number of classes in mapping: {len(some_mapping)}")
        logging.info(f"Unique intent labels in batch: {torch.unique(intent_labels)}")

        # Check the weights being used
        if self.focal_loss and self.focal_loss.alpha is not None:
            logging.info(f"Shape of class_weights (alpha): {self.focal_loss.alpha.shape}")
            logging.info(f"Class weights (top 5): {self.focal_loss.alpha[:5]}")
            logging.info(f"Class weights sum: {self.focal_loss.alpha.sum()}")
            logging.info(f"Class weights mean: {self.focal_loss.alpha.mean()}")
