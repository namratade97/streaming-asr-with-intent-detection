...
#         if len(indices) < 2:
#             # With only one sample, inside similarity doesn't really make sense.
#             intent_ratios[intent] = {"inside": None, "outside": None, "ratio": None}
#             continue

#         # Compute inside similarity: average over all pairs in this group
#         inside_sum = 0.0
#         inside_count = 0
...
#         # Compute outside similarity: for each index in the group, average similarity with all indices not in the group
#         outside_sum = 0.0
#         outside_count = 0
...
# #     # Append top 5 closest sentences (as before)
...
# #     # Write out the records with closest sentences
# #     # write_jsonl(records, args.output)
# #     # print(f"Result saved to {args.output}")

# #     # Compute intent-level similarity ratios
# #     intent_ratios = compute_intent_similarity_ratios(records, sim_matrix)
# #     print("\nIntent similarity ratios (inside average similarity / outside average similarity):")
# #     for intent, stats in intent_ratios.items():
# #         if stats["ratio"] is None:
# #             print(f"  Intent '{intent}': Not enough data to compute ratio.")
# #         else:
# #             print(f"  Intent '{intent}': inside avg = {stats['inside']:.3f}, outside avg = {stats['outside']:.3f}, ratio = {stats['ratio']:.3f}")
...
#     # Append top 5 closest sentences (optional, keep or remove as needed)
#     for i, record in enumerate(records):
#         scores = sim_matrix[i].copy()
...
#     # Compute and sort intent-level similarity ratios
#     intent_ratios = compute_intent_similarity_ratios(records, sim_matrix)
...
#     # print("\nIntent similarity ratios (sorted by ascending ratio):")
#     # for intent, stats in sorted_ratios:
#     #     if stats["ratio"] is None:
#     #         print(f"  Intent '{intent}': Not enough data to compute ratio.")
#     #     else:
#     #         print(f"  Intent '{intent}': inside avg = {stats['inside']:.3f}, outside avg = {stats['outside']:.3f}, ratio = {stats['ratio']:.3f}")
...
#     # Sort intents by frequency (descending)
